...
export default function UsersPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [meRes, listRes] = await Promise.all([
        ...
        fetch("/api/ops/users", { credentials: "include" }),
      ]);
      if (!meRes.ok) {
      ...
      if (!listRes.ok) {
        ...
        setLoadError(
          typeof err?.error === "string" ? err.error : "Could not load users.",
        );
        setUsers([]);
        return;
      }
      const listData = (await listRes.json()) as { users?: DirUser[] };
  ...
  const canInvite = Boolean(me && canProvisionUsers(me.role));
      ...
      <section className="border border-zinc-200 rounded-sm bg-white overflow-hidden">
        <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between px-6 py-4 border-b border-zinc-200 bg-zinc-50/80">
          ...
          <p className="text-[11px] font-bold tracking-widest text-zinc-500 uppercase">
            Total users: {loading ? "—" : formatTotal(users.length)}
          </p>
        </div>
              ...
              {loading ? (
                <tr>
                  <AdminTableCell
                    density="comfortable"
                    colSpan={4}
                    className="py-16 text-center text-zinc-400 font-medium text-xs uppercase tracking-widest"
                  >
                    Loading directory…
                  </AdminTableCell>
                </tr>
      ...
      {inviteOpen ? (
        ...
        >
          <div className="w-full max-w-md rounded-sm border border-zinc-200 bg-white p-6 shadow-xl">
            <h3 className="text-lg font-black tracking-tight text-zinc-900 mb-1">
              New operator
            </h3>
            <p className="text-xs text-zinc-500 mb-6">
              Creates an account and returns a one-time invite code.
            </p>
            ...
            {inviteResult ? (
              <div className="space-y-4">
                <p className="text-sm text-zinc-700">
                  User{" "}
                  <span className="font-semibold">{inviteResult.username}</span>{" "}
                  created. Copy the codes below; they will not be shown again.
                </p>
              ...
              <form onSubmit={submitInvite} className="space-y-4">
                ...
                <div className="flex gap-2 pt-2">
                  ...
                  <button
                    type="submit"
                    disabled={inviteSubmitting || assignableRoles.length === 0}
